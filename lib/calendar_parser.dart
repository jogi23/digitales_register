...
// The portal's calendar weeks (`api/calendar/student`) as calendar days.
// Apart from the provider so the background check can read a week, too,
// without the app's state around it.

/// The days of one loaded week, by date. [previous] are the days as they
/// were loaded before; whether a file was downloaded is taken over from them.
Map<UtcDateTime, CalendarDay> parseCalendarWeek(
  Map<String, dynamic> data, {
  Map<UtcDateTime, CalendarDay> previous = const {},
}) {
  return data.map(
    (k, dynamic e) {
...
LessonContentSubmission? _parseLessonContentSubmission(
  ...
  final fileAvailable = oldHour?.lessonContents.any(
        (content) => content.submissions.any(
          (s) =>
              s.originalName == originalName && s.id == id && s.fileAvailable,
        ),
      ) ??
      false;
